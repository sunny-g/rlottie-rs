...
fn main() {
    ...
    // FIXME: hack around local <angled> include, which panics bindgen
    {
        ...
        f = f.replace("#include <rlottiecommon.h>", "#include \"rlottiecommon.h\"");
    ...
    // let link_lib = lib_path.join("librlottie.a");

    // println!("out dir: {}", std::env!("OUT_DIR"))

    // println!("cargo:rustc-link-search=native={}", link_lib.display());
    // println!("cargo:rustc-link-lib=librlottie");
}
